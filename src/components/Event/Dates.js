...
import { isMultidate, compareDates} from '../../helpers/dates';
...
class EventDate extends Component {
  ...
  keepEdits = (date, type) => {
    ...
    if (this.props.type === 'sell' && this.props.event['dates']['stay'][type] === "") {
      this.props.event['dates']['stay'][type] = date.toLocaleString('en-US').split(',')[0];
    }
  ...
  render() {
    ...
    // console.log(this.props.type, this.props.event['dates'][this.props.type]);
    ...
    return (<div className={`event-dates ${stay} ${!compareDates(this.props.end, this.props.start)? ' problem ' : '' }`}>
      {
        this.props.editable 
